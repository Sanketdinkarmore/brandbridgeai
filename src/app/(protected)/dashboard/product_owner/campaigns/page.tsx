...
export default function ProductOwnerCampaignsPage() {
  ...
  return (
    <div>
      <PageHeader title="Campaigns" subtitle="Track product promotion campaigns" action={
        <button onClick={() => setShowForm(!showForm)} className="bb-btn-primary flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm"><Plus size={16} /> New Campaign</button>
      } />
      {showForm && (
        <form onSubmit={handleCreate} className="bb-glass mb-6 rounded-2xl p-6 space-y-4">
          ...
          <button type="submit" className="bb-btn-primary rounded-xl px-4 py-2 text-sm">Create</button>
        </form>
      )}
      {campaigns.length === 0 ? (
        <EmptyState icon={Megaphone} title="No campaigns" description="Create a promotion campaign for your products." />
      ) : (
